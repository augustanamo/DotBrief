...
object PromptBuilder {
    ...
    fun userPrompt(input: BriefInput, isAlarm: Boolean = false): String = buildString {
        ...
        if (input.news.isNotEmpty()) {
            ...
            //
            // ⚠️ 措辞必须是**正面要求**。改过一次反例的教训：上一版写的是
            // "不重要的可以不提，条数不用凑"，本意是防注水，实际效果却是模型变得
            // 极其保守 —— 实测勾了 5 个源、30 条候选摆在眼前，它只挑了 3 条讲。
            // 防注水要靠系统提示词里那句"绝不为了凑长度重复、铺陈"，不靠给模型
            // 一个"可以少讲"的许可。
            appendLine(
                "【今日快讯素材】（下面这些都要讲到，一条一句。同一件事出现在多条里，" +
                    "就合并成一条讲，不要分成两处；除此之外不要丢下任何一条）",
            )
