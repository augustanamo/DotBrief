...
object Defaults {
    ...
    val SYSTEM_PROMPT: String = """
        ...
        3. 严格按这个顺序讲：时间与问候 -> 纪念日（今天有生日／纪念日就第一时间单独说一句，它比任何安排都重要）-> 天气 -> 紧急行程（航班／高铁，必须念清时间与提醒）-> 待办与快递包裹（取件码要一字一顿念准）-> 新闻快讯（素材里值得说的都讲，不必凑条数）。
        ...
        8. 篇幅完全跟着素材走：素材多就把该说的说清楚，素材少就三五句收住。绝不为了凑长度重复、铺陈或加客套话，也不要为了显得短而丢掉素材里真正要紧的事。
     ...
     *
     * 兜底值取得很宽（从前最宽的一档才 450 字）：宁可放开，也不要拿一个
     * 早就不在界面上、用户没法再调的数字去砍他的内容。
     */
    ...
    const val LEGACY_MAX_CHARS: Int = 800
    ...
    /**
     * 正文的兜底上限（字符数）：**不是配额，是防失控**。
     *
     * 篇幅已经不设档位（跟着素材走），但模型偶尔会吐出一大段跑偏的内容，
     * 代码层得有一个"绝不可能念这么长"的天花板。800 字约合三分半，
     * 比过去最宽的一档（450 字）还宽松 —— 正常内容永远碰不到它。
     */
    const val BRIEF_SANITY_MAX_CHARS: Int = 800
