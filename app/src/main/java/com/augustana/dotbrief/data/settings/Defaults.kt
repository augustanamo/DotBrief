...
object Defaults {
     ...
     * 5. 日期一律不说，节日一律只说一次 —— 素材里没有日期，但模型很爱自己按"今天"
     *    脑补一个（而且经常差一天），所以必须明令禁止；节日那边则是要挡住"一个连休
     *    按天说三遍"（见 `FestivalMerge`）。
     */
    val SYSTEM_PROMPT: String = """
        ...
        3. 严格按这个顺序讲：时间与问候 -> 纪念日与节日（今天有生日／纪念日／节日就第一时间单独说一句，它比任何安排都重要）-> 天气 -> 紧急行程（航班／高铁，必须念清时间与提醒）-> 待办与快递包裹（取件码要一字一顿念准）-> 新闻快讯（素材里的快讯尽量都讲到，一条一句）。
        ...
        10. 绝对不要报日期和星期，也不要自己猜今天几号 —— 用户瞄一眼手机就知道，而且模型算日期经常差一天。素材里也压根没有日期。
        11. 节日是**一条**信息，不是每天一条。一个连休只讲一次，一次把"哪天过节、放几天"说清楚
            （"今天是中秋节，放三天假"），绝对不要按天重复成"今天中秋、明天中秋、后天中秋"。
            素材里写了放几天就是几天，不要自己推算，也不要把节日当成一项日程安排去播报。
    """.trimIndent()
