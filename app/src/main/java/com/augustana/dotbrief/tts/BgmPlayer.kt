 ...
 * [TAIL_ENTRY_MS] 处"，剩下这几秒由系统放完，播完自然静音，
 * 全程不需要进程醒着。收尾**不再做音量淡出**（改成浮到 [TAIL_VOLUME]）——
 * 淡不淡只是观感，"到点没声"这条底线由音频自己保证。
 *
 * ⚠️ 挪位置必然产生一个**接缝**：前后是两段毫不相干的乐句，硬接在一起
 * 听上去就是"音乐断了一下"（主人原话："BGM 是断掉了又重新响两秒"）。
 * 躲不开，但可以把它藏进一条音量曲线里 —— 见 [tailOutInternal]。
 *
...
class BgmPlayer(private val context: Context) {
    ...
    /**
     * 走尾巴：关掉循环、把播放位置挪到"距曲尾 [TAIL_ENTRY_MS] 处"。
     *
     * 挪位置而不是从当前位置继续放，是因为曲子已经循环了不知道多少轮，
     * 剩下的长度不可控 —— 只有"距末尾固定多少秒"才能保证到点就停。
     *
     * ⚠️ 这一跳有个必须处理的副作用：前后是**两段毫不相干的乐句**，硬接在一起
     * 听上去就是"音乐断了一下"（主人原话："BGM 断掉了又重新响两三秒"）。
     * 挪位置躲不开，但可以把它藏起来 —— 见下面的注释。
     */
    private fun tailOutInternal(onDone: () -> Unit) {
        ...
        if (duration <= TAIL_ENTRY_MS) {
            // 拿不到时长（个别编码会返回 -1），或曲子比尾巴窗口还短：退回立即停。
            stopInternal()
            onDone()
            return
        }
        ...
        runCatching {
            //
            // ⚠️ **接缝要藏在这三行里。** 这里把播放位置从"当前正在响的地方"平移到
            // 曲尾前 [TAIL_ENTRY_MS]，前后两段乐句没有任何关系 —— 直接切，耳朵收到的
            // 就是"音乐断了一下，然后重新开始"。主人确实就是这么描述的。
            //
            // 挪位置本身躲不开（控制时长全靠它），但**听感上的连续性可以绕开**：
            // 先抹掉音量 -> 挪 -> 再从静音里升回来。这样前后不再是"同一段音乐裂开"，
            // 而是"上一句收了、下一句起来" —— 一个正常的乐句边界，不像故障。
            //
            // 三步必须在同一个 runnable 里连着做完，中间不能插入 await：
            // 静音状态每多留一帧，用户听到的"空档"就多一帧。
            media.isLooping = false
            media.setVolume(0f, 0f)
            volume = 0f // 让下面的 [fadeTo] 从 0 起步
            media.seekTo((duration - TAIL_ENTRY_MS).toInt())
        }
        ...
        //
        // 末段不做淡出：音频已被挪到曲尾前、会自己播完，最后那一下收束是素材自带的
        // （[TAIL_ENTRY_MS] 的实测表里能看到它怎么落到数字静音）。而"到点一定没声"
        // 这条底线本来就由素材自己保证，不该再指望动画（见上面那条 VSYNC 的坑）。
        // 这里同时是"接缝 -> 收尾"的那条淡入曲线，见上面那一段。
        fadeTo(TAIL_VOLUME, TAIL_RAMP_MS)
        // 兜底：completion 回调万一不来（音频异常），也不能让音乐继续循环。
        // 和上面一样，它在休眠时不执行，但那时的风险已经由"音频播完"兜住了。
        scheduleWatchdog(TAIL_ENTRY_MS + FALLBACK_MS)
    }
    ...
    companion object {
         ...
         * "从曲尾往前数 N 秒"从来都不等于"能听到 N 秒音乐"，见 [TAIL_ENTRY_MS]。
         *
         * ⚠️ 它**不等于**"音频从曲尾前多远开始放" —— 那由 [TAIL_ENTRY_MS] 说了算，
         * 因为"从曲尾往前数 N 秒"从来不等于"能听到 N 秒音乐"（里有实测数据）。
         * 这里是**想要的结果**，那里是**素材能兑现多少**，两者都对不上 digits 才正常。
         */
         ...
         * 尾巴从**距曲尾多远**切入（毫秒）。
         *
         * ## 这个值必须量出来，不能由 [TAIL_HOLD_MS] 反推
         *
         * 最初是 `TAIL_HOLD_MS + 2500`，前提假设是"从曲尾往前 N 秒 = 能听到 N 秒音乐"。
         * 实测（`tools/probe_bgm.py`，0.25 秒一帧）把这个假设推翻了：
         *
         * | 距曲尾 | rms | 说明 |
         * |---|---|---|
         * | 7.50s | -10.2 dB | 主体还满着 |
         * | **7.00s** | **-9.5 dB** | ← **本常量的落点，全曲最饱满的一帧** |
         * | 6.50s | -18.9 dB | ← **旧落点**：一跳就掉 9.4 dB，听着就是"音乐断了" |
         * | 3.00s | -33.7 dB | 基本听不见了 |
         * | 2.50s | -84.0 dB | 等于没了 |
         * | ≤1.75s | 数字静音 | 一直静到曲尾 |
         *
         * 素材自己的收尾（最后 2.5 秒）**不算"陪伴"** —— 那段时间里音乐其实已经没了。
         *
         * ## 为什么是 7.0 秒而不是 6.5
         *
         * 落点处的音量必须和它**前一秒齐平**：两处都在 -10 dB 上下，于是"平移到这儿"
         * 不再是变小，只剩下换了个乐句。旧的 6.5 秒正好跨过了 outro 的起跌点
         * （-9.5 → -18.9 dB），所以主人听到的是"断了又重新响"。
         *
         * 从这里到声音消失约 4.2 秒，正对 [TAIL_HOLD_MS]。
         *
         * ⚠️ **换 `bgm.mp3` 就要重新量这个值** —— 它和 [DRUM_DROP_MS] 一样绑在素材上，
         * 只是那条管开场、这条管收尾。
         */
        private const val TAIL_ENTRY_MS = 7000L

        /**
         * 人声退场后的收尾音量。
         *
         * ⚠️ 不是"比 [DUCKED_VOLUME] 高一档"这么简单 —— 还得补上**素材自己的落差**：
         * [TAIL_ENTRY_MS] 那段落点的 rms 平均只有 -19 dB，比主体（-13 dB）低 6 dB。
         * 再乘 0.6 的话，落到耳朵里几乎和垫底没区别，"还陪你一会儿"就白做了。
         *
         * 取 0.85：接近滿幅，让最后这几秒**清楚地**响出来。尾巴那几秒里没有人声，
         * 不存在抢注意力的问题，所以这里不必像垫底那样让着它。
         */
        private const val TAIL_VOLUME = 0.85f

        /**
         * 从静音浮回 [TAIL_VOLUME] 的时长。
         *
         * 慢一点是有意的：这一条同时是"藏接缝"的**唯一**手段（见 [tailOutInternal]）。
         * 太快的话音乐像被人拍了一下，掩盖不住 [TAIL_ENTRY_MS] 那一次挪位置的痕迹。
         */
        private const val TAIL_RAMP_MS = 400L
