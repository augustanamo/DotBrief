...
图标不是手摆坐标画出来的，而是把 DotMatrixArt.render() 的那套数学照搬过来：
    ...
    三段色阶   = base #202317 -> mid #7E8C14 -> core #F2FF3C（默认色相 64°）
...
FOREGROUND_COMMENT = """<!--
  ...
  下面每一颗点的位置、半径、颜色，都由 DotMatrixArt.render() 的那几条式子算出来：
      ...
      三段色阶   = #202317 -> #7E8C14 -> #F2FF3C（默认色相 64° 那套）
  ...
  整片 225 颗点一颗不少，只是 5 列 x 7 行的那 18 颗给满亮度（#F2FF3C），
  其余压到 42% 强度、留在暗橄榄色区间。于是"点阵"和"字母"是同一张图的两个层次，
  而不是"字母贴在一片底噪上"。
  ...
  只有字母的话，图标就是"黑底上飘着一个黄绿色的 D"，跟组件没关系了。
